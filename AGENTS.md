...
codebase analyzer for TypeScript/JavaScript — to surface dead code, duplication, complexity
hotspots and architecture-boundary violations that ESLint cannot see (it analyzes the
full module graph instead of one file at a time).

Config: `.fallowrc.json` (auto-generated by `fallow init`, monorepo-aware).
`.fallow/` is gitignored (cache + local data).

### Common scripts

```bash
npm run fallow              # Combined report: dead-code + dupes + health
npm run fallow:dead-code    # Unused files / exports / types / deps / class members
npm run fallow:dupes        # Cross-file duplicated blocks
npm run fallow:health       # Cyclomatic / cognitive complexity hotspots
npm run fallow:audit        # Scoped to changed files (good for PRs)
npm run fallow:fix          # Preview safe auto-fixes (dry-run)
```

Fallow is **advisory**, not a CI gate — its findings are informational. When AI-generated
changes land, run `npm run fallow:audit` locally before opening a PR to catch new dead
code and duplication early.
